...
from discovery import create_supabase_client, ingest_sources
...
def run_agent(supabase=None):
    supabase = supabase or create_supabase_client()
    print("Starting the Job Opportunity Agent daily run...")

    try:
        # 1. Pull the structured upstream feed and optional mailbox signals.
        ingest_sources(supabase)

        # 2. Review newly requested companies (DeepSeek decides approve/reject + grabs favicon)
        print("Reviewing pending company requests...")
        review_pending_companies(supabase)

        # 3. Scrape approved company pages, diffing against the cached content
        print("Scraping company pages...")
        scrape_companies(supabase)

        # 4. Check approved companies' LinkedIn pages for actionable posts
        print("Checking company LinkedIn posts...")
        scrape_company_posts(supabase)

        # 5. Scrape LinkedIn job searches (logged-out, simple diff against cache)
        print("Scraping LinkedIn searches...")
        scrape_searches(supabase)

        # 6. Evaluate new jobs against user profiles
...
def evaluate_and_notify(supabase=None):
    """
